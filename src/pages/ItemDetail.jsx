...
function ItemDetail({ item, error, isLoading, onBack, t, locale, onLocaleChange }) {
  ...
  return <div className="page-shell"><Navbar t={t} locale={locale} onLocaleChange={onLocaleChange} /><main className="detail-page"><button className="back-link" type="button" onClick={onBack}><span aria-hidden="true">←</span> {t.detail.back}</button><section className="detail-hero"><div className="detail-copy"><p className="eyebrow">{t.itemDetail.eyebrow} #{String(item.id).padStart(3, '0')}</p><h1>{item.localizedName}<em>.</em></h1><div className="type-list"><span>{formatName(item.category)}</span></div><p className="detail-description">{item.effect || t.itemDetail.noEffect}</p>{item.cost > 0 && <div className="measurements"><div><small>{t.itemDetail.cost}</small><strong>₽{item.cost}</strong></div></div>}</div><div className="detail-art"><div className="art-ring" /><img src={item.image} alt={item.name} /></div></section></main><Footer t={t} /></div>
}
